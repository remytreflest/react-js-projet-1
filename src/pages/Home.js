...
const Home = () => {
    const compdata = file.competences.map((competence) => {
        return (
            <li key={competence.id}>
                <Link to={`/competences/${competence.id}`}>{competence.name}</Link>
            </li>
        )
    })
    ...
    const formData = file.formations.map((formation) => {
        return (
            <Card
                ...
                experience={formation.description}
                data={formation.date}
            />
        )
    })
    ...
    return (
        <>
            ...
            <Section title='Expériences'>{expData}</Section>

            <Section title='Formations'>{formData}</Section>
            ...
            <Section title='Url de votre site ou git'>
                <a href={file.urlGit} target='_blank' rel='noreferrer'>
                    Dépôt Git
                </a>
            </Section>
        </>
